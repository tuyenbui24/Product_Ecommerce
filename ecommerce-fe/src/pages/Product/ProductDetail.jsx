...
import { getProduct as getProductDetail, getProductSizes } from "@/api/adminApi";
...
export default function ProductDetail() {
  ...
  const handleAddToCart = async () => {
    ...
    if (sizes.length && stockOfSelected <= 0) return toast.error("Size đã hết hàng");
  ...
  if (!p) return <div className="p-6 text-center text-gray-500">Đang tải...</div>;
    ...
    <div className="max-w-6xl mx-auto w-full p-4 md:p-6">
      <div className="grid md:grid-cols-2 gap-6 bg-white rounded-xl p-4 md:p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="text-rose-600 text-2xl font-semibold">{fmtPrice(p.price, true)} đ</div>

          {!!sizes.length && (
            <div className="space-y-2">
              <div className="font-medium">Chọn size</div>
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        active ? "border-amber-500 bg-amber-50 text-amber-700" : "border-gray-300 hover:bg-gray-50",
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <button onClick={dec} className="h-9 w-9 rounded border hover:bg-gray-50" aria-label="Giảm">–</button>
              <input
                type="number"
                min={1}
                value={qty}
                onChange={(e) => {
                  ...
                  const v = Math.max(1, Math.min(Number(e.target.value) || 1, cap));
                  setQty(v);
                }}
                className="h-9 w-16 rounded border text-center"
              />
              <button onClick={inc} className="h-9 w-9 rounded border hover:bg-gray-50" aria-label="Tăng">+</button>
              {sizes.length > 0 && <span className="text-sm text-gray-500">Kho còn: {stockOfSelected}</span>}
            </div>
          </div>

          <button
            onClick={handleAddToCart}
            disabled={sizes.length > 0 && (!selectedSize || stockOfSelected <= 0)}
            className="h-11 w-full md:w-auto px-6 rounded-md border-2 border-amber-500 text-amber-700 font-medium transition-colors hover:bg-amber-50 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            Thêm vào giỏ hàng
          </button>
